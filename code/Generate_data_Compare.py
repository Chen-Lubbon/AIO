'''
    用于训练对比网络的从data读取图像和深度，生成不同的恶劣天气图像和对应的maps，并按照9:1随机划分为train和test，保存到data文件夹
'''
...
if __name__ == "__main__":
    ...
    splits = ["train", "test"]
    subdirs = ['Dehaze', 'Derain', 'Deblur', 'lowlight', 'lowlight', 'lowlight']
    subsub_names = ['degraded', 'landscape']
    
    for split in splits:
        for subdir in subdirs:
            for subsub in subsub_names:
                os.makedirs(f"data_compare/{split}/{subdir}/{subsub}", exist_ok=True)
    ...
    with tqdm(enumerate(zip(images, depths)), desc='Generating', total=total_images, mininterval=5) as pbar:
        for index, (img_file, depth_file) in pbar:
            ...
            base_save_path = f"data_compare/{split_dir}"
            current_method_saving = subdirs[method_index]
            cv2.imwrite(f"{base_save_path}/{current_method_saving}/degraded/{img_file}", cv2.cvtColor((J_out * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
            cv2.imwrite(f"{base_save_path}/{current_method_saving}/landscape/{img_file}", cv2.cvtColor((img * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))