"""_summary_
由于在generate data弄了train test分类，因此不需要再在train文件夹做random split，需要修改load data的方案
"""
import torch as t
...
def load_test_datas(num_data):
    ...
    '''
    testI = t.stack([dataset_val[i][0] for i in range(num_data)], dim=0)
    testT = t.stack([dataset_val[i][1] for i in range(num_data)], dim=0)
    testM = t.stack([dataset_val[i][2] for i in range(num_data)], dim=0)
    testA = t.stack([dataset_val[i][3] for i in range(num_data)], dim=0)
    testLabels = t.stack([dataset_val[i][4] for i in range(num_data)], dim=0)
...
def write_log(savepath, message):
    ...
    with open(f"{savepath}/details.txt", 'w') as f:
        ...
        f.write(f'{message[1]/message[2]} of data used for training \n')
        f.write(f'Epochs: {message[3]}, Batch size: {message[4]}, Save interval: {message[5]}\n')
        if message[6] is not None:
            f.write(f'loaded model from {message[6]}\n')
        f.write(f'loss weights for I, I0, T, M, A: {message[7]}\n')
        f.close()
...
def train(device, train_loader, val_loader, Testdata, test_indx, savepath, epochs=100, save_interval=25, model_path = None):
    ...
    messages = (test_indx, len(train_loader.dataset), len(val_loader.dataset), epochs, args.Batch_size, save_interval, model_path, lossweights)
...
if __name__ == "__main__": 
    ...
    epoch = 200
    trainers = [[f'--Epoch', f'{epoch}', '--Root_train', 'data/train', '--Root_test', 'data/test', '--Batch_size', '3', '--Save_interval', '50']]
    
    for i in range(trainers.__len__()):
        sys.argv += trainers[i]
        parser = argparse.ArgumentParser(description='Net for Speckle Experiment')
        parser.add_argument('--Root_train', type=str, default='data/train', help='Path to scatter data')
        parser.add_argument('--Root_test', type=str, default='data/test', help='Path to scatter data for testing')
        ...
        rdseed = 4
        random.seed(rdseed)
        dataset_train = LoadData(root=args.Root_train, random_sequence=True, random_seed=rdseed , total_num=None ) # add total_num to limit the dataset size for quick test
        dataset_val = LoadData(root=args.Root_test, random_sequence=True, random_seed=rdseed , total_num=None )
        
        train_loader = DataLoader(dataset_train, batch_size=args.Batch_size, shuffle=True)
        val_loader = DataLoader(dataset_val, batch_size=args.Batch_size, shuffle=False)
        
        print(f'train dataset size: {len(dataset_train)}, test dataset size: {len(dataset_val)}')

        transform = T.Compose([T.ToTensor()])
        
        dataset_indx_val = dataset_val.__get_indices__()
        test_num = 8
        test_indx = dataset_indx_val[:test_num]
        ...
        print( f'Root path {args.Root_train} ', f'Output path {savepath} ', f'Using model {args.Load_model_path} ')
